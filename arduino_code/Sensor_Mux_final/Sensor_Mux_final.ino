...
void calibrateSensors() {
  ...
  for(int i = 1; i <= 180; i++) {
    TGS2620_R0 += readMux(0);
    MQ2_R0 += readMux(1);
    MQ3_R0 += readMux(2);
    MQ4_R0 += readMux(3);
    TGS2600_R0 += readMux(4);
    TGS2602_R0 += readMux(5);
    TGS2603_R0 += readMux(6);
    TGS2610_R0 += readMux(7);
    MQ5_R0 += readMux(8);
    MQ6_R0 += readMux(9);
    MQ7_R0 += readMux(10);
    MQ8_R0 += readMux(11);
  ...
  R0_values[0] = TGS2620_R0;
  R0_values[1] = MQ2_R0;
  R0_values[2] = MQ3_R0;
  R0_values[3] = MQ4_R0;
  R0_values[4] = TGS2600_R0;
  R0_values[5] = TGS2602_R0;
  R0_values[6] = TGS2603_R0;
  R0_values[7] = TGS2610_R0;
  R0_values[8] = MQ5_R0;
  R0_values[9] = MQ6_R0;
  R0_values[10] = MQ7_R0;
  R0_values[11] = MQ8_R0;
...
void loop() {
  ...
  if (millis() - dt > 2000) {
    ...
    for (int i=0; i<13; i++){
      ...
      String query = "INSERT INTO test_db1.Sensor4 (Sensor, Value, stamp) VALUES ('" + channelNames[i] + "','" + String(out_values[i],9) + "',NOW())";
